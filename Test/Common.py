...
def PackHeadInternal(cmd, body_size, tid):
	MAGIC_NUM="INTE"
	head=struct.pack("!4s2i1q",MAGIC_NUM,body_size,cmd,tid)
	return head
...
def ReqAndRspTid(SVR_ADDR, CMD, REQ, RSP, TID):
        ...
        print ("[REQ:cmd=%d,body_size=%d]\n%s" %(CMD, len(send_buf), str(REQ)))
        recv_buf = SendAndRecvData(SVR_ADDR, send_buf)
        MAGIC_NUM_,BODY_SIZE_,CMD_,TID_=struct.unpack("!4s2i1q", recv_buf[0:20]);
        #4. unserialize
        RSP.ParseFromString(recv_buf[20:])
        print("[RSP:magic=%s,cmd=%d,body_size=%d,tid=%ld]\n%s" %(MAGIC_NUM_, CMD_, BODY_SIZE_, TID_, str(RSP)))
